...
if isdirectory(stdpath("config")."/snippets")
	if expand('%:e') == 'h'
		if filereadable(stdpath("config")."/snippets/snippet.h")
			nnoremap <silent> <buffer> <leader>sn :execute("-1read ".stdpath("config")."/snippets/snippet.h")<CR> Wl"=expand('%:t:r')<C-M>pgUiWjl"=expand('%:t:r')<C-M>pgUiWji
		else
			echo "Snippet not available"
		endif
	elseif expand('%:e') == 'cpp'
		if filereadable(stdpath("config")."/snippets/snippet.cpp")
			nnoremap <silent> <buffer> <leader>sn :execute("-1read ".stdpath("config")."/snippets/snippet.cpp")<CR> jjji<Tab>
		else
			echo "Snippet not available"
		endif
	endif
