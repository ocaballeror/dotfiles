...
if isdirectory(stdpath("config")."/snippets")
	if filereadable(stdpath("config")."/snippets/snippet.java")
		nnoremap <leader>sn :execute("-1read ".stdpath("config")."/snippets/snippet.java")<CR> eela<C-R>=expand('%:t:r')<CR><Esc>jji<Tab><Tab>
	else
