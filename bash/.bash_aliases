...
alias k='gitk --all &'
alias klogs='kubectl logs -f --tail 300 $(kubectl get pod | grep "$(basename $PWD)" | cut -d" " -f1) "$(basename $PWD)"'
...
alias ppu='pipenv --rm; pipenv install --dev && pipenv run pip install black debugpy flake8 ptpython pynvim python-lsp-server pylsp-mypy pdbpp && pipenv run pip install -e ~/Stuff/autoimport'
alias pppw='ppw'
...
alias venv='rm -rf .venv && virtualenv .venv && source .venv/bin/activate && pip install black flake8 ptpython pynvim python-lsp-server pylsp-mypy pdbpp && { [ ! -f requirements.txt ] || pip install -r requirements.txt; }'
alias vi='vim'
alias vimm='vim'
alias vmi='vim'
